...
* ##### I'm seeing the "Standup is disabled for this channel" message on opening the standup modal.
    ...
    The channel standup is disabled. Run `/standup config` and enable standup from the form that opens.

* ##### I run `/standup`, but no dialog opens for submitting standup.
...
* ##### I run `/standup config` but no configuration dialog opens.
